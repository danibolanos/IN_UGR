...
if __name__ == '__main__': 
  ...
  '''
  categories = ['legal_ownership_status', 'ground_floor_type',
  'roof_type', 'foundation_type', 'other_floor_type', 'plan_configuration',
  'position','land_surface_condition']
  X = pd.get_dummies(data_x[categories])
  X_tst = pd.get_dummies(data_x_tst[categories])
  '''
  X = data_x.values
  ...
  '''
  print("------ Catboost...")
  cbc = CatBoostClassifier(n_estimators=1000, learning_rate=0.12, eval_metric='TotalF1',
  ...
  submission(X_train, y_train, X_tst, cbc, index_categories)
  '''
  ...
  best_param4 = clf.best_params_['od_pval']
