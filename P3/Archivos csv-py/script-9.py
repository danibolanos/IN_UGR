...
if __name__ == '__main__': 
  ...
  print("------ Catboost...")
  cbc = CatBoostClassifier(n_estimators=450, learning_rate=0.12, eval_metric='TotalF1',
  od_pval=0.001, od_type='IncToDec', random_seed=76592621, bootstrap_type='Bayesian',
  max_depth=12, best_model_min_trees=250)
  #cbc, y_train_clf, y_test_clf = validacion_cruzada(cbc, X_sample, y_sample, 2)
  print("------ Generando submission...")
  submission(X_sample, y_sample, X_tst, cbc)
  '''
  fit_alg = RandomForestClassifier(max_features = 'sqrt', criterion='gini', n_jobs=-1, 
  ...
  clf = GridSearchCV(fit_alg, param_dist, verbose=1, cv=5, scoring='f1_micro', n_jobs=2)
  clf = clf.fit(X,y)
  best_param1 = clf.best_params_['max_depth']
  best_param3 = clf.best_params_['n_estimators']
  best_param2 = clf.best_params_['class_weight']
