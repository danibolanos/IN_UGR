...
if __name__ == '__main__': 
  ...
  cuts = pd.cut(data_x['geo_level_1_id'], 21)
  data_x.drop(['geo_level_1_id'], axis = 1, inplace = True)
  le = LabelEncoder()
  cuts = le.fit_transform(cuts)
  data_x['geo_level_1_id'] = cuts.T

  cuts = pd.cut(data_x_tst['geo_level_1_id'], 21)
  data_x_tst.drop(['geo_level_1_id'], axis = 1, inplace = True)
  le = LabelEncoder()
  cuts = le.fit_transform(cuts)
  data_x_tst['geo_level_1_id'] = cuts.T

  cuts = pd.cut(data_x['geo_level_2_id'], 23)
  data_x.drop(['geo_level_2_id'], axis = 1, inplace = True)
  le = LabelEncoder()
  cuts = le.fit_transform(cuts)
  data_x['geo_level_2_id'] = cuts.T

  cuts = pd.cut(data_x_tst['geo_level_2_id'], 23)
  data_x_tst.drop(['geo_level_2_id'], axis = 1, inplace = True)
  le = LabelEncoder()
  cuts = le.fit_transform(cuts)
  data_x_tst['geo_level_2_id'] = cuts.T
  ...
  print("------ LightGBM...")
  lgbm = lgb.LGBMClassifier(n_estimators=2695, learning_rate=0.09, max_bin=300, num_leaves=32,
                            objective='multiclassova', random_state=76592621, n_jobs=2)
